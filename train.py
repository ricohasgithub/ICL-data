...
def experiment(transformer_lr, mlp_lr):
    epochs = 50000
    ...
    optim = torch.optim.SGD(
        [
            {
                "params": [param[1] for param in transformer_params],
                "lr": transformer_lr,
            },
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0:
            ...
            acc_ic2 = accuracy(
                model, test_inputs_ic2, test_labels_ic2, flip_labels=True
            )
...
if __name__ == "__main__":
    ...
    tlr = float(sys.argv[1])  # transformer learning rate
    mlr = float(sys.argv[2])  # mlp learning rate

    experiment(transformer_lr=tlr, mlp_lr=mlr)
